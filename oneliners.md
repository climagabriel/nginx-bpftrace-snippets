...
bpftrace -e 'uprobe:/usr/sbin/nginx:ngx_shmtx_lock { @t[pid]=nsecs } uprobe:/usr/sbin/nginx:ngx_shmtx_unlock /@t[pid]/ { if(nsecs-@t[pid]>=10000000000){printf("pid %d held shmtx for %llums\n",pid,(nsecs-@t[pid])/1000000)} delete(@t[pid]) }'



========== OPEN / FILE / CACHE FILE OPS ==========
...
timeout 60 bpftrace -e 'tracepoint:syscalls:sys_enter_io_submit { @[comm] = count(); }'

timeout 60 bpftrace -e 'tracepoint:syscalls:sys_enter_io_submit { @start[tid] = nsecs; } tracepoint:syscalls:sys_exit_io_submit /@start[tid]/ { @hgram = hist ((nsecs - @start[tid]) / 1000)}'
...
timeout 60 bpftrace -e 'uprobe:/usr/sbin/nginx:ngx_ssl_handshake { @start[tid] = nsecs; } uretprobe:/usr/sbin/nginx:ngx_ssl_handshake /@start[tid]/ { @hgram = hist ((nsecs - @start[tid]) / 1000); @count = count(); delete(@start[tid]); }'

timeout 60 bpftrace -e 'uprobe:/usr/sbin/nginx:ngx_epoll_process_events { @start[tid] = nsecs; } uretprobe:/usr/sbin/nginx:ngx_epoll_process_events /@start[tid]/ { @epoll_iteration_duration = hist((nsecs - @start[tid]) / 1000); delete(@start[tid]); }'
...
timeout 10 bpftrace -e 'uretprobe:/opt/gcore/openresty12532-ssl352-gcdn/openssl352/lib/libssl.so.3:SSL_get_error { @[retval] = count(); }'



========== QUIC ==========

bpftrace -e 'uretprobe:/usr/sbin/nginx:ngx_quic_guard_allow_sendmsg {printf("%d\n", retval);}'



========== SIGNALS ==========

bpftrace -e 'tracepoint:signal:signal_deliver /pid == 19534  && args->sig == 1/ { printf("%s SIGHUP\n", strftime("%H:%M:%S", nsecs)); }'
